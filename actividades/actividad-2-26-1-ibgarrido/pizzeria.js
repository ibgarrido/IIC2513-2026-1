...
function calculatePrice({
  ...
  */
  ...
  let price = 5;
  price += nPepperonis * 0.5;
  price += nBasils * 0.3;
  price += nMushrooms * 0.1;
  if (hasSauce) {
    price += 1;
  }
  if (hasCheese) {
...
function displayInvoice({
  ...
  */
  ...
  if (hasSauce) {
    ingredients.sauce = "Yes";
  } else {
    ingredients.sauce = "No";
  }
  if (hasCheese) {
    ingredients.cheese = "Yes";
  } else {
    ingredients.cheese = "No";
  }

  const summaryContainer = document.getElementById("order-summary");

  const limpieza = summaryContainer.replaceChildren();

  const ulElement = document.createElement("ul");

  /* Disclaimer: la AI me explicó la lógica de como utilizar AppendChild ya que no pude
  ir a clases y no está subida la clase aún de manejo del DOM */
  for (const key in ingredients){
    const liIngredient = document.createElement("li");
    const textContent = `${key}: ${ingredients[key]}`;
    const textNode = document.createTextNode(textContent);
  
  liIngredient.appendChild(textNode);
  ...
  const ptotaltext = document.createTextNode(`Total: $${total}`);
